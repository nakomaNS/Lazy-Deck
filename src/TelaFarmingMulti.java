...
public class TelaFarmingMulti {
  ...
  public void mostrar(StackPane rootDoDashboard) {
    ...
    for (JogoFila jogo: filaJogos) {
      HBox item = criarItemVisual(jogo, overlay);
      jogo.container = item;
  ...
  private HBox criarItemVisual(JogoFila jogo, StackPane parentOverlay) {
    ...
    jogo.lblStatus = new Label("Aguardando");
    jogo.lblStatus.setStyle("-fx-text-fill: " + COR_TEXTO_SECUNDARIO + "; -fx-font-size: 12px;");
    statusRow.getChildren().addAll(jogo.statusDot, jogo.lblStatus);

    jogo.lblCartas = new Label(jogo.dropsAtuais + " drops");
