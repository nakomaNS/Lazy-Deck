...
public class TelaFarming {
    ...
    public void mostrar(StackPane rootDashboard) {
        ...
        imgCapa.setFitHeight(280); imgCapa.setPreserveRatio(true);
        Rectangle clip = new Rectangle(186, 280); clip.setArcWidth(16); clip.setArcHeight(16); imgCapa.setClip(clip);
        ...
        lblTitulo.setWrapText(true); lblTitulo.setTextAlignment(javafx.scene.text.TextAlignment.CENTER);

        HBox statusBox = new HBox(10); statusBox.setAlignment(Pos.CENTER);
        statusDot = new Circle(4); statusDot.setFill(Color.web(COR_VERDE_NEON)); statusDot.setEffect(new DropShadow(10, Color.web(COR_VERDE_NEON)));
        lblStatus = new Label("Iniciando..."); lblStatus.setStyle("-fx-text-fill: " + COR_VERDE_NEON + "; -fx-font-size: 13px; -fx-font-weight: bold;");
        statusBox.getChildren().addAll(statusDot, lblStatus);

        HBox timerBox = new HBox(5); timerBox.setAlignment(Pos.CENTER);
        ...
        double size = 11;
        iconEdit.setPrefSize(size, size); iconEdit.setMinSize(size, size); iconEdit.setMaxSize(size, size);
